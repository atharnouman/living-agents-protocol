...
Gemini ran prompt 13 (Task B) against commit adf9d6d and filed `inbox/2026-09-08_gemini_hostile-review.md`: eight findings on the newest perimeter (THREAT-MODEL, LIP-3 v0.3, the fuzzer). Every one was reproduced before acceptance; all eight accepted, none rejected. Two severities are lowered here with reasons, and two of the reviewer's proposed diffs were wrong (corrected below). Fixes in both ports with regression tests, each confirmed to fail against pre-patch code (Python by stash at the time; the Node tests were re-verified against an adf9d6d worktree in the review pass that followed). Spec -> v0.4.10, LIP-3 -> v0.4.

- **F1 [FATAL->SERIOUS] idempotency `claim()` could not tell a fresh reservation from an in-flight one.** It returned `null` both for a new slot and for a slot already reserved-but-not-completed, so two concurrent mutating requests both saw "free" and both executed. Lowered to SERIOUS: the helper is documented single-process and non-production and no shipped path calls it yet, but the API was broken for its stated purpose — and the project had presented it as the row-9 defence in THREAT-MODEL.md, with a round-4 regression test that asserted `null` on both calls and so enshrined the bug. Fix: three-state return (`reserved` / `in_flight` / `completed`+receipt); callers proceed only on `reserved`.
