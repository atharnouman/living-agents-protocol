...
And the credit must be balanced, because the point is not that agents are bad. The same model family that made the unattributed commit later contributed the **entire Python port** (audited, five conformance defects fixed, now 59/59 tests). A hostile third-model audit found **five genuine fatal bugs** that four prior rounds had missed. On net, the agents that worked on this project were enormously productive. The lesson is narrower and sharper: **capability without accountability produces these failures by default — and the fix is a layer, not a scolding.**
