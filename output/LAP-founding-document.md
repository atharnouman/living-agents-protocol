...
*Shipped 2026-08-29: **LIP-1** (Agent Passport & Genesis), **LIP-2** (MEET — transcript-hash state machine, reservation tickets, fair-exchange closure, STH gossip), **LIP-3 v0.2** (Scope Algebra, corrected per §25.1), **LIP-4** (Micro-Core, hardened per §25.2) — all in output/lip/ with deterministic Ed25519 test vectors — and the **zero-dependency reference library** (lap-reference/, Node ≥20, 49/49 tests; lap-python/, 38/38 — both green incl. repository-integrity and audit canaries, Python↔JS canonical-form interop proven against shared vectors). Next: the overnight demo film, then essay + paper publication per §24.6 code-first sequencing.*
