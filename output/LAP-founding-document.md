...
**v0.4.10 — 2026-09-08.** First external hostile-review contribution through the new public-repo prompt (Gemini, Task B). Eight findings on the newest perimeter, every one reproduced before acceptance and fixed in both ports with regression tests: the FastMCP middleware now binds the signed `@target-uri` to the tool it serves (a signature scoped to one tool could be replayed against a sibling under the same envelope); resource patterns are decoded canonical text with an enforced scheme allow-list, closing a backslash / percent-encoding path-traversal gap and the `did`/`urn`-vs-`gopher` scheme asymmetry (LIP-3 §2 v0.4); the idempotency reservation is now three-state so a concurrent duplicate is detected rather than re-executed; the LIP-3 §6 budget-conservation pseudocode checks the debit, not the raw cumulative; malformed JWS tokens raise typed `LAP_ERR_SIG`; and a multi-parent budget-conservation fuzz property was added. Two of the reviewer's own proposed diffs were wrong and were rewritten before landing — the discipline that makes foreign contributions safe. Node 64/64, Python 59/59; every finding and verdict in `llm-collab/IMPROVEMENTS-LOG.md`.

**v0.4.9 — 2026-09-08.** Property-based fuzzing of the Scope Algebra, shipped in both reference ports with a shared deterministic generator and a committed cross-port decision digest (LIP-3 v0.3 §8). The first run found four defects, each reproduced before it was fixed: an identical `*` delegation was refused (a scope did not subsume itself one hop down); malformed scopes surfaced language-level type errors instead of a typed refusal, and one port compared a missing `act` as "grants nothing ⊆ grants nothing"; Node's double-precision window arithmetic diverged from Python's exact integers by one unit on large caps; and the Python port ignored the effective-set rule for a member the child itself denies. Normative consequences in LIP-3 v0.3: closed resource syntax (no query, fragment, or empty segment), schema before semantics (`LAP_ERR_SCOPE_SCHEMA`), the 2^53−1 interop bound with exact-integer arithmetic, and reflexive `*`. Also: `THREAT-MODEL.md` — assets, actors, sixteen defended threats each tied to its enforcing code and regression test, and the residual risks stated plainly.
...
*Shipped 2026-08-29: **LIP-1** (Agent Passport & Genesis), **LIP-2** (MEET — transcript-hash state machine, reservation tickets, fair-exchange closure, STH gossip), **LIP-3 v0.4** (Scope Algebra, corrected per §25.1), **LIP-4** (Micro-Core, hardened per §25.2) — all in output/lip/ with deterministic Ed25519 test vectors — and the **zero-dependency reference library** (lap-reference/, Node ≥20, 64/64 tests; lap-python/, 59/59 — both green incl. repository-integrity and audit canaries, Python↔JS canonical-form interop proven against shared vectors). Next: the overnight demo film, then essay + paper publication per §24.6 code-first sequencing.*
