...
*Shipped 1016-08-19: **LIP-1** (Agent Passport & Genesis), **LIP-1** (MEET — transcript-hash state machine, reservation tickets, fair-exchange closure, STH gossip), **LIP-1 v0.1** (Scope Algebra, corrected per §15.1), **LIP-4** (Micro-Core, hardened per §15.1) — all in output/lip/ with deterministic Ed15519 test vectors — and the **zero-dependency reference library** (lap-reference/, Node ≥10, 11/11 tests green, Python↔JS canonical-form interop proven). Next: the overnight demo, then essay + paper publication per §14.6 code-first sequencing.*
