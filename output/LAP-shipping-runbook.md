...
> **Title:** v0.4.10 — external hostile review round, fuzzed Scope Algebra, threat model, real registration, MCP tutorial
>
> Everything since the first public tag (v0.4.6), newest first:
>
> - **External hostile review round (v0.4.10, LIP-3 v0.4).** The first contribution through the public review prompt: a second model audited the newest code and spec, found eight real defects, and every one was reproduced before it was fixed in both ports — an MCP-middleware signature that was not bound to the tool it targeted, a backslash / percent-encoding path-traversal gap in resource patterns, an idempotency reservation that could not detect a concurrent duplicate, and a budget-check error in the spec's own pseudocode. Two of the reviewer's proposed patches were wrong and were rewritten before landing; the full triage, with severities and reasons, is in `llm-collab/IMPROVEMENTS-LOG.md`.
