...
- **Spec v0.4.9** — 26 sections, hardened by **five adversarial review rounds** (~150 verified fixes merged) across three model families: Claude red teams, external Gemini reviews, and a purely hostile GPT-5.6 security audit — every finding verified before merge, rejections logged with reasons.
...
- **Two interoperating implementations** — a zero-dependency Node library (61/61 tests) and a Python port with FastMCP middleware (55/55) — proven byte-identical against shared test vectors.
- **A networked demo** across two real OS processes over real sockets (IPv4 and IPv6): the crash is a genuine process kill, the recovery a genuine restart with persistent identity.
- **`lap-git`** — the same mechanisms applied to commits (an agent passport plus a path envelope enforced before a commit exists), built after AI agents working in this very repo made an unattributed, out-of-scope commit — documented honestly in a case study.
...
*Code, spec, demo and papers: https://github.com/atharnouman/living-agents-protocol · live demo: https://atharnouman.github.io/living-agents-protocol/*
