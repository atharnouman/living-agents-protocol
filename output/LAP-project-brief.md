...
- **Spec v0.4.10** — 26 sections, hardened by **six adversarial review rounds** (~160 verified fixes merged) across three model families: Claude red teams, external Gemini reviews, and a purely hostile GPT-5.6 security audit — every finding verified before merge, rejections logged with reasons.
...
- **Two interoperating implementations** — a zero-dependency Node library (64/64 tests) and a Python port with FastMCP middleware (59/59) — proven byte-identical against shared test vectors.
