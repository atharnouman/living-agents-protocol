...
- **Spec v0.4.8** — 26 sections, hardened by **five adversarial review rounds** (~150 verified fixes merged) across three model families: Claude red teams, external Gemini reviews, and a purely hostile GPT-5.6 security audit — every finding verified before merge, rejections logged with reasons.
...
- **Two interoperating implementations** — a zero-dependency Node library (51/51 tests) and a Python port with FastMCP middleware (45/45) — proven byte-identical against shared test vectors.
