...
- **Spec v0.4.7** — 26 sections, hardened by **five adversarial review rounds** (~150 verified fixes merged) across three model families: Claude red teams, external Gemini reviews, and a purely hostile GPT-5.6 security audit — every finding verified before merge, rejections logged with reasons.
...
- **Two interoperating implementations** — a zero-dependency Node library (49/49 tests) and a Python port with FastMCP middleware (43/43) — proven byte-identical against shared test vectors.
