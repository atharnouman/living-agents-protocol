...
*What the Living Agents Protocol defends against, how the reference implementations prove it, and what it deliberately does not defend against. Written for spec v0.4.10; every LIP change should touch this file.*
...
| 3 | A captured header pair replayed elsewhere, or with a different body | Holder-of-key: the request-signature key is derived from the verified passport's `sub`, never from the request (F2); the RFC 9421 base must cover method, target, content digest, and passport hash (F3); the FastMCP middleware binds the signed `@target-uri` to the tool it serves (v0.4.10) | `verifyRequestSignature` / `verify_request_signature`, `mcp_middleware.py` | `round4.test.js`, `test_audit_round4.py::test_f2_request_signature_key_from_sub`, `test_middleware.py::test_mcp_signature_target_is_bound_to_the_tool`, CONFORMANCE check 8, the MCP CI job |
...
| 5 | A delegate widens its authority: sub-verb escalation, path-prefix escape, wildcard widening, dot-segment or encoded-separator traversal, window inversion, cross-currency caps, deny-list bypass, budget replication across siblings, depth or decay loosening | The Scope Algebra (LIP-3): registry DAG with no string prefixing; whole-segment matching with terminal-only wildcards; closed URI syntax (scheme allow-list; no percent-encoding, backslashes, or control characters — v0.4); two-dimensional caps; conservation debited in parent-window units (F7); schema before semantics (F8, v0.3); caps inside the single-scope predicate (F9); effective-set counterparties | `algebra.js`, `algebra.py` | `test/algebra.test.js`, `round4.test.js`, `tests/test_algebra.py`, and the property-based fuzzers `test/fuzz.test.js` / `tests/test_fuzz.py` (nine properties, including multi-parent conservation, plus a committed cross-port decision digest) |
...
| 9 | Duplicate execution of a retried mutating call | Atomic `(agent, Idempotency-Key)` claim before execution (F6); the claim is three-state so a concurrent in-flight duplicate is refused, not re-executed (v0.4.10); distributed backing is a production MUST | `IdempotencyCache` in both ports | `test_audit_round4.py::test_f6_idempotency_claim_three_state` |
...
- Six adversarial review rounds across three model families, about 160 verified fixes, with rejections logged and reasoned.
...
- External hostile review (Gemini, 2026-09-08), the first contribution through the public-repo prompt: eight findings, all reproduced and fixed in both ports — an unbound signature target in the MCP middleware, a backslash/percent path-traversal gap, an idempotency reservation blind to a concurrent duplicate, a normative budget-check error, and four smaller typed-error and coverage issues (v0.4.10). Two of the reviewer's own proposed patches were wrong and were rewritten before landing.
